# Sub-Session 02: Worker Node Overview

This sub-session is part of 01-kubernetes-architecture.

## Goal

kubelet, container runtime, kube-proxy, CNI.
...
```bash
kubectl apply -f subsessions/02-worker-node-overview/
```
...
```bash
kubectl delete -f subsessions/02-worker-node-overview/ --ignore-not-found
```
