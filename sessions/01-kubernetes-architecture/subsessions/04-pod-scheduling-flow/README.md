# Sub-Session 04: Pod Scheduling Flow

This sub-session is part of 01-kubernetes-architecture.

## Goal

from YAML to a running Pod.
...
```bash
kubectl apply -f subsessions/04-pod-scheduling-flow/
```
...
```bash
kubectl delete -f subsessions/04-pod-scheduling-flow/ --ignore-not-found
```
