# Sub-Session 06: Managed Cluster View

This sub-session is part of 01-kubernetes-architecture.

## Goal

what EKS manages and what worker nodes run.
...
```bash
kubectl apply -f subsessions/06-managed-cluster-view/
```
...
```bash
kubectl delete -f subsessions/06-managed-cluster-view/ --ignore-not-found
```
