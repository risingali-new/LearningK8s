# Sub-Session 07: Pod Status Debugging

This sub-session is part of 03-configuration-and-pod-lifecycle.

## Goal

phase, conditions, container states, events.
...
```bash
kubectl apply -f subsessions/07-pod-status-debugging/
```
...
```bash
kubectl delete -f subsessions/07-pod-status-debugging/ --ignore-not-found
```
