# Sub-Session 02: Init Containers

This sub-session is part of 03-configuration-and-pod-lifecycle.

## Goal

run setup before the main container starts.
...
```bash
kubectl apply -f subsessions/02-init-containers/
```
...
```bash
kubectl delete -f subsessions/02-init-containers/ --ignore-not-found
```
